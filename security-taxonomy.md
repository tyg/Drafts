# JavaScript Modularity and Security<br/>A taxonomy of issues and approaches

by Mark S. Miller

The discovery of the [Meltdown and Spectre](https://meltdownattack.com/) side channels show that secrets are harder to protect than expected. Some have overreacted, claiming that [language-based security is impossible](https://mobile.twitter.com/domenic/status/1298045068633407490). However, “security” is an overly broad umbrella term. After presenting a taxonomy of issues and approaches, this essay explains what forms of security are possible using which technologies and what forms of security are necessary for large-scale software engineering. We conclude with implications for the evolution of the JavaScript language.
...
We avoid this I/O problem in ocap languages that are *[fully virtualizable](https://en.wikipedia.org/wiki/Popek_and_Goldberg_virtualization_requirements)* in the sense first discovered for processor architectures, where there is a strict separation between the *user-mode* purely computational language vs the *system-mode* (host) objects that provide I/O access to the external world. [SES](https://github.com/tc39/proposal-ses) leverages JavaScript’s almost perfect conformance to this model and repairs the remaining gap.
...
Some have said we must “[put untrusted modules in a sandboxed process](https://mobile.twitter.com/domenic/status/1298045620993904640).” This is either a gross misunderstanding of what developers do, a novel meaning of the word “untrusted”, or a blame-the-victim strategy to avoid responsibility for shipping a platform that cannot be programmed safely at any speed.
...
Starting with [EcmaScript 5](https://www.youtube.com/watch?v=Kq4FpMe6cRs&list=PLKr-mvz8uvUg70w0yKGfytaDqxiIBNo_L), JavaScript has gotten much more secure and much better at supporting [large scale software engineering](https://www.youtube.com/watch?v=9WdbTucMaRo&list=PLzDw4TTug5O3vIAd4IR1Gp5t_46co_dv9), largely through the same mechanisms introduced for “both” purposes — repaired scoping and encapsulation, promises, proxies, weakmaps, classes with private fields, tagged template literals, separated modules, and more. These mechanisms serve the same abstract goal, to enable more effective composition by minimizing the hazards of destructive interference. However, they are described in the disjoint terminologies of two different communities, causing misunderstanding.

Worse, “security” words have political costs for browser makers that they understandably wish to avoid. Their classification of bugs draws a hard line — as it should! — between “security critical” vulnerabilities vs “normal” bugs in JavaScript implementations. The “security critical” category must have tremendously more ritual and cost around the management of these bugs and the urgency to fix them. Browser makers need to focus these expensive efforts somehow. They chose to focus on threats to the browser’s official “security” model— the “same origin policy” — making [no distinction between integrity and confidentiality](https://mobile.twitter.com/domenic/status/1298045068633407490). Some such choices must be made. Given browser history, their choices may be reasonable and we do not wish to second guess them.

However, JavaScript is also used on non-browser hosts with different tradeoffs. On [Node servers](https://medium.com/@mikesamuel/security-at-speed-f6b87a3aceba), [TC53](https://www.youtube.com/watch?v=pMyGLmfgU5A&list=PLzDw4TTug5O3vIAd4IR1Gp5t_46co_dv9) and [Moddable embedded devices](https://www.youtube.com/watch?v=-2UnCfy5NSs&list=PLzDw4TTug5O25J5M3fwErKImrjOrqGikj), [Agoric blockchains](https://www.youtube.com/watch?v=YcWXqHPui_w&list=PLhuBigpl7lqth_Ow_eQWZs7NFxmeDw9W8), and [MetaMask browser extensions](https://www.youtube.com/watch?v=4A01rYb5Fpg&list=PLzDw4TTug5O25J5M3fwErKImrjOrqGikj), fine grain integrity has a huge payoff. None of these has nor wants the browser’s official security model — the “same origin policy”. JavaScript the standard can have one set of mechanisms that serve everyone’s shared software engineering needs for integrity, modularity, and abstraction. At the same time, each of the players in this ecosystem already makes different decisions about what security claims to make about these mechanisms, and what rituals to attach to what claims. The JavaScript standard has always supported a diversity of such decisions and will continue to do so.
